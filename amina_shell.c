...
void init_shell() {
    ...
    const char *yellow = "\033[38;5;228m";
    const char *blue_gray = "\033[38;5;67m";
    ...
    printf("%s", gray);
    printf("╔════════════════════════════════════════════╗\n");
    printf("║       %sAminaShell | Terminal Personnel%s      ║\n", yellow, gray);
    printf("╚════════════════════════════════════════════╝\n");

    if (timestamp) {
        printf("   %s🕒 Date :%s %s%-25s%s\n", yellow, gray, blue_gray, timestamp, gray);
    }

    char *username = getenv("USER");
    if (username) {
        printf("   %s👤 Utilisateur :%s %s%-20s%s\n", yellow, gray, blue_gray, username, gray);
    }
...
int main(void) {
    ...
    while (running) {
        const char *blue_gray = "\033[38;5;67m";     // élégant
        const char *yellow = "\033[38;5;228m";       // jaune pâle
        const char *reset = "\033[0m";               // reset couleurs

        printf("%s%s%s:%s%s> %s", blue_gray, prompt(), reset, yellow, get_current_dir(), reset);
